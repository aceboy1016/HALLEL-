...
function processMessage(message) {
  try {
    ...
    if (storeFieldMatch) {
      ...
      // 優先順位：渋谷 > 代々木上原 > 中目黒 > 恵比寿 > 半蔵門
      if (storeName.includes('渋谷')) {
        detectedStore = 'shibuya';
      } else if (storeName.includes('代々木上原')) {
    ...
    if (!detectedStore) {
      ...
      // 日本語店舗名で検索（優先順序：渋谷最優先）
      if (body.includes('渋谷')) {
        detectedStore = 'shibuya';
      } else if (body.includes('代々木上原')) {
        detectedStore = 'yoyogi-uehara';
