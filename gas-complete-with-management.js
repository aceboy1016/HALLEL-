 ...
 * 【初回セットアップ手順】
 *
 * 1. このコード全体をGASエディタに貼り付け
 * 2. 以下の3つの関数を順番に実行：
 *
 *    ① deleteAllTriggers()
 *       → 古いトリガーを全て削除
 *
 *    ② fullCleanupHallelLabels()  ※オプション
 *       → HALLELラベルを全てクリーンアップ
 *       注意: ラベルが削除されるので、必要な場合のみ実行
 *
 *    ③ setupTomorrowMorningSync()
 *       → 明日の朝6時に forceFullSync() を自動実行
 *
 * 3. あとは寝るだけ！
 *    明日の朝6時に全メールが自動的にVercelシステムに同期されます
 *
 * 【実行方法】
 * - GASエディタで関数名を選択
 * - 実行ボタン（▶️）をクリック
 ...
 * 【トラブルシューティング】
 * - エラーが出た場合は、ログを確認
 * - Gmail API quota exceeded: 明日まで待つ
 * - 認証エラー: GASの承認画面で許可
 */
