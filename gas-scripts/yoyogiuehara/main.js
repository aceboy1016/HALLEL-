...
const CONFIG = {
  ...
  INCLUDE_KEYWORD: '代々木上原店',  // この店舗のメールのみ処理
  EXCLUDE_KEYWORDS: ['恵比寿店', '半蔵門店', '渋谷店', '中目黒店'],  // 他店舗を確実に除外
  DEFAULT_ROOM: '代々木上原店',
  ...
  CALENDAR_ID: null,
  // 処理済みラベル
  LABEL_NAME: 'HALLEL_処理済み_代々木上原',
};

/**
 * 【店舗固有】部屋名を抽出（代々木上原店: 2枠あるが区別なし）
 */
function extractStudio(body) {
  // パターン1: 「設備： 代々木上原店 パーソナルジム (1)」形式
  const match = body.match(/設備[：:]\s*代々木上原店\s*([^\s(]+)/);
 ...
 * @param {string} storeKeyword - 店舗キーワード（例：'代々木上原'）
 * @param {string} includeKeyword - 含むべきキーワード（例：'代々木上原店'）
  ...
  try {
    ...
    for (const thread of threads) {
      ...
      for (const msg of thread.getMessages()) {
        ...
        Logger.log(`--- 最強店舗判定結果 [${msgId.substring(0, 8)}] ---`);
