...
const CONFIG = {
  ...
  INCLUDE_KEYWORD: '中目黒店',  // この店舗のメールのみ処理
  EXCLUDE_KEYWORDS: ['恵比寿店', '半蔵門店', '渋谷店', '代々木上原店'],  // 他店舗を確実に除外
  DEFAULT_ROOM: 'フリーウエイトエリア',
  API_URL: 'https://hallel-shibuya.vercel.app/api/gas/webhook',
  API_KEY: 'Wh00k@2025!Secure$Token#ABC123XYZ',
  // カレンダー同期（null = 同期しない）
  CALENDAR_ID: null,
  // 処理済みラベル
  LABEL_NAME: 'HALLEL_処理済み_中目黒',
};

/**
 * 【店舗固有】エリア名を抽出（中目黒店: フリーウエイトエリア・格闘技エリア）
 */
function extractStudio(body) {
  // パターン1: 「設備： 中目黒店 フリーウエイトエリア（奥） (1)」形式
  const match = body.match(/設備[：:]\s*中目黒店\s*(フリーウエイトエリア|格闘技エリア)/);
  if (match) {
    return match[1].includes('フリーウエイト') ? 'フリーウエイトエリア' : '格闘技エリア';
  }

  // パターン2: 本文中のキーワード
  if (body.includes('フリーウエイトエリア')) return 'フリーウエイトエリア';
  if (body.includes('格闘技エリア')) return '格闘技エリア';
 ...
 * @param {string} storeKeyword - 店舗キーワード（例：'中目黒'）
 * @param {string} includeKeyword - 含むべきキーワード（例：'中目黒店'）
    ...
    for (const thread of threads) {
      for (const msg of thread.getMessages()) {
        ...
        if (!storeValidation.isValid) {
          Logger.log(`詳細: ${JSON.stringify(storeValidation.details, null, 2)}`);
        }
  ...
  try {
    ...
    for (const thread of threads) {
      ...
      for (const msg of thread.getMessages()) {
        ...
        // 【新ロジック】最強店舗判定（多層防御）
        const storeValidation = validateStoreEmail(msg.getSubject(), body, lines, CONFIG.STORE_KEYWORD, CONFIG.INCLUDE_KEYWORD, CONFIG.EXCLUDE_KEYWORDS);
        if (!storeValidation.isValid) {
          Logger.log(`スキップ: ${storeValidation.reason}`);
          continue;
        }
        Logger.log(`店舗判定成功: ${storeValidation.reason} (信頼度: ${storeValidation.confidence}%)`);
