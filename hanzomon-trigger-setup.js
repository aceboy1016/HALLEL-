 ...
 * トリガーを設定（1時間ごと）
 */
function setupHanzomonTrigger1hour() {
  // 既存のトリガーを削除
  deleteAllTriggers();
...
function processNewReservations() {
  ...
  Logger.log('【半蔵門店：新規メール処理開始】');
  Logger.log('='.repeat(80));

  try {
    // 過去1時間のメールを取得
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
    const threads = GmailApp.search(`from:noreply@em.hacomono.jp subject:hallel after:${Math.floor(oneHourAgo.getTime() / 1000)}`);
