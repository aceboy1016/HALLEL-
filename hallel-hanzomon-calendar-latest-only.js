/**
 * HALLEL半蔵門店 - 最新状態のみをカレンダーに反映
 *
 * 過去の全メールから最新状態のみを抽出してカレンダーに反映
...
function syncLatestReservationsToCalendar() {
  ...
  Logger.log('【半蔵門店：最新状態のみをカレンダーに反映】');
  Logger.log('='.repeat(80));

  try {
    const calendar = CalendarApp.getCalendarById(CONFIG_HANZOMON.CALENDAR_ID);
    if (!calendar) {
...
function getAllReservationEmails() {
  const threads = GmailApp.search(CONFIG_HANZOMON.SEARCH_QUERY);
...
function applyToCalendarWithRateLimit(calendar, latestReservations) {
  ...
  for (let i = 0; i < latestReservations.length; i++) {
    ...
    try {
      ...
      // バッチ処理：5件ごとに3秒待機
      if ((i + 1) % CONFIG_HANZOMON.BATCH_SIZE === 0) {
        Logger.log(`\n  ⏸️ ${i + 1}件処理完了。${CONFIG_HANZOMON.WAIT_TIME_MS / 1000}秒待機中...\n`);
        Utilities.sleep(CONFIG_HANZOMON.WAIT_TIME_MS);
      }
    ...
    for (let event of existingEvents) {
      ...
      if (nameMatch && isHallelEvent && startMatch && endMatch) {
        if (title === eventTitle) {
          ...
          // 旧形式または部屋名が違う重複イベントを削除
          Logger.log(`    🔄 重複削除: ${title}`);
          event.deleteEvent();
        }
      }
    }
...
function testLatestSync() {
  Logger.log('🧪 テスト実行: 過去7日間のみ処理');

  // CONFIG を一時的に変更
  const originalQuery = CONFIG_HANZOMON.SEARCH_QUERY;
  CONFIG_HANZOMON.SEARCH_QUERY = `${originalQuery} newer_than:7d`;

  const result = syncLatestReservationsToCalendar();

  // CONFIG を元に戻す
  CONFIG_HANZOMON.SEARCH_QUERY = originalQuery;
